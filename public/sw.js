...
// were removed while chasing a real, CI-reproduced bug -- and are staying
// out even though that removal turned out not to be the actual fix, purely
// as a defense-in-depth simplification (installing shouldn't need to
// eagerly succeed at fetching things a page will fetch anyway; fewer things
// that can fail install is a good default regardless).
//
// The real bug, fully root-caused: two or more ids generated by the SAME
// app/icon.tsx generateImageMetadata array (e.g. /icon/192 and the maskable
// variant that too used to be a third id of that file), requested
// concurrently, intermittently came back with a PNG of the CORRECT declared
// byte length but corrupted, undecodable content -- confirmed directly
// against a real browser and a real production build, repeatedly, only
// under concurrency, never on an isolated request, and confirmed to be
// unrelated to this install-time precache list specifically (it reproduced
// from plain page-level `fetch()` calls with no precaching involved at
// all). That profile -- right length, wrong bytes, only across sibling ids
// of one generateImageMetadata route requested at once -- points at a
// Next.js 16.3.0/Turbopack bug in how it serves concurrent requests to the
// same dynamic image-metadata route, not at anything in this app's own
// rendering (lib/icon-mark.tsx's IconArtwork decodes correctly every time
// it's requested outside that specific concurrency shape). The actual fix
// was moving the maskable icon out to its own independent route
// (app/icon-maskable/route.tsx) so it no longer shares a generateImageMetadata
// array -- and therefore whatever internal state Next was corrupting --
// with any sibling id; see that route's header for the full writeup.
//
// classifyRequest() below still classifies `/icon/*`, `/apple-icon`, and
// `/icon-maskable` as "static" regardless of this list, so cacheFirst()
// still caches each one the first time a page actually requests it.
